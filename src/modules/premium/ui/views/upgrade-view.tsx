            ...
            <div className="mt-4 flex-1 flex flex-col gap-y-10 items-center">
                <h5 className="font-medium text-2xl md:text-3xl">
                    ...
                    <span className="font-semibold text-primary">
                        {currentSubscriptions?.name ?? "Free"}
                    </span>{" "}plan
                </h5>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {products.map((product)=>{
                        const isCurrentProduct = currentSubscriptions?.id === product.id
                        const isPremium = !!currentSubscriptions
...
export const UpgradeLoadingView = () => {
    return (
        <LoadingState title="Loading..." description="This may take a few seconds...." />
    )
}

export const UpgradeErrorView = () => {
    return (
        <ErrorState title="Error Loading Subscriptions" description="Please try again later" />
    )
}
